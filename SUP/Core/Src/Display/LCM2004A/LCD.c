...
//############################################################################################
/* Pin definitions */
#define LCD_RS_LOW              PCF_GPIO_WritePin(_LCD_RS_PORT, _LCD_RS_PIN,GPIO_PIN_RESET)
#define LCD_RS_HIGH             PCF_GPIO_WritePin(_LCD_RS_PORT, _LCD_RS_PIN,GPIO_PIN_SET)
#define LCD_RW_LOW              PCF_GPIO_WritePin(_LCD_RW_PORT, _LCD_RW_PIN,GPIO_PIN_RESET)
#define LCD_RW_HIGH             PCF_GPIO_WritePin(_LCD_RW_PORT, _LCD_RW_PIN,GPIO_PIN_SET)
#define LCD_E_LOW               PCF_GPIO_WritePin(_LCD_E_PORT,  _LCD_E_PIN,GPIO_PIN_RESET)
#define LCD_E_HIGH              PCF_GPIO_WritePin(_LCD_E_PORT,  _LCD_E_PIN,GPIO_PIN_SET)
...
void  LCD_Delay_us(uint16_t  us)
{
  ...
  uint32_t  StartMicros = HAL_GetTick();
  HAL_Delay(us);
  //while((StartMicros - HAL_GetTick()< us));
}
//############################################################################################
...
static void LCD_Cmd4bit(uint8_t cmd)
{
	PCF_GPIO_WritePin(_LCD_D7_PORT, _LCD_D7_PIN, (GPIO_PinState)(cmd & 0x08));
	PCF_GPIO_WritePin(_LCD_D6_PORT, _LCD_D6_PIN, (GPIO_PinState)(cmd & 0x04));
	PCF_GPIO_WritePin(_LCD_D5_PORT, _LCD_D5_PIN, (GPIO_PinState)(cmd & 0x02));
	PCF_GPIO_WritePin(_LCD_D4_PORT, _LCD_D4_PIN, (GPIO_PinState)(cmd & 0x01));
	LCD_E_BLINK;
}
//############################################################################################
static void LCD_CursorSet(uint8_t col, uint8_t row)
